import type { AuditEvent, AuditRecorder } from '../modules/case-management/domain/ports/AuditRecorder.js';
 ...
 * twin of `auditRecorderAdapter.ts`): implements case-management's OWN
 ...
 * it is the one legal seam where a cross-module import (case-management's
 * port depending on the audit module's use case) is allowed by
 * `eslint-plugin-boundaries`.
 *
 * `tx` is case-management's OWN opaque `Transaction`; `recordAuditLog` wants
 ...
 * nominal types for this module, mirroring identity-access's adapter.
 */
export function createCaseManagementAuditRecorderAdapter(
  recordAuditLog: ReturnType<typeof createRecordAuditLogUseCase>,
): AuditRecorder {
  return {
    async record(event: AuditEvent, tx?: CaseManagementTransaction): Promise<void> {
