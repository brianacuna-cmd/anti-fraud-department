...
export interface EnrichedFinturuCustomer {
  ...
  readonly wallets: readonly any[];
  readonly transfers: readonly any[];
  readonly stripe: Record<string, any> | null;
...
export function createGetFinturuDirectoryUseCase(deps: GetFinturuDirectoryDeps) {
  return async function getFinturuDirectory(input: GetFinturuDirectoryInput = {}): Promise<readonly EnrichedFinturuCustomer[]> {
    const orgId = input.organizationId ?? input.auth?.organizationId ?? deps.defaultOrganizationId ?? '019d7e58aed0777318d11d4d';

    // 1. Fetch data from Finturu API and existing cases in parallel
    const [customers, wallets, transfers, stripeCustomers, stripeTransfers, existingCasesPage] = await Promise.all([
    ...
    // Map existing cases by customerId / bridgeUserId
    ...
    const enrichedCustomers: EnrichedFinturuCustomer[] = [];
    ...
    for (const customer of customers) {
      ...
      const idUser = customer.idUser ? String(customer.idUser).trim() : (customer.idUserBridge ?? `cust_${Date.now()}`);
      ...
      const existingCase =
        (idUser ? caseByCustomerKey.get(idUser) : null) ??
        (bridgeUserId ? caseByCustomerKey.get(bridgeUserId) : null) ??
        (email ? caseByCustomerKey.get(email) : null) ??
        null;
