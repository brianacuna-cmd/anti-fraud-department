import { brand, type Brand } from '../../../../../shared/kernel/Brand.js';
import { generateObjectIdHex, isObjectIdHex } from '../../../../../shared/kernel/ObjectIdHex.js';
import { invariantViolation } from '../../errors/CaseManagementError.js';

export type EvidenceId = Brand<string, 'EvidenceId'>;

/** Validates a raw id coming from persistence, DTOs, or route params. */
export function createEvidenceId(value: string): EvidenceId {
  if (!isObjectIdHex(value)) {
    throw invariantViolation('EvidenceId must be a 24-character hexadecimal ObjectId', { value });
  }
  return brand<string, 'EvidenceId'>(value);
}

/** Mints a fresh id: a 24-char hex string the Mongo mapper stores as `ObjectId`. */
export function generateEvidenceId(): EvidenceId {
  return brand<string, 'EvidenceId'>(generateObjectIdHex());
}
