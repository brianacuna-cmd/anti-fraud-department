...
/**
 * Mongo adapter for `CaseRoutingRuleRepository` (design: "CaseRoutingRules").
 * T1 auto-routing only reads ACTIVE rules for an organization, ordered by
 * `CreatedAt` ascending so `RouteCase`'s first-match-wins semantics are
 * deterministic (oldest rule takes precedence). The
 * `{ OrganizationId, Status }` index keeps this off a collection scan.
 */
