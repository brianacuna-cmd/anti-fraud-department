...
import { invariantViolation } from '../../../../domain/errors/PrivacyError.js';

/**
 * Parses transport input against a zod schema, translating any failure into
 * `INVARIANT_VIOLATION` (mirrors sar's `parseRequest`) so it flows through
 * the same `errorHandler` as every other domain error.
 */
