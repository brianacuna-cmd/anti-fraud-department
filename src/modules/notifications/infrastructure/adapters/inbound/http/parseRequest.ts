...
import { invariantViolation } from '../../../../domain/errors/NotificationsError.js';

/**
 * Parses transport input against a zod schema, translating any failure into
 * `INVARIANT_VIOLATION` (copied from identity-access's own `parseRequest`)
 * so it flows through the same `errorHandler` as every other domain error.
 */
