...
function buildUser(id: string, organizationId = ORG_1, email = 'user-1@example.com'): User {
  return User.create({
    id: createUserId(id),
    organizationId,
...
describe('UserRepository (port contract, via InMemoryUserRepositoryFactory fake)', () => {
  it('returns null from findById when nothing has been saved', async () => {
    ...
    const result = await repository.findById(createUserId(oid('missing')));
  ...
  it('overwrites the stored user when saving the same id again', async () => {
    ...
    const repository = factory.forTenant(ORG_1);
    await repository.save(buildUser(oid('user-1')));
    const suspended = (await repository.findById(createUserId(oid('user-1'))))!.transitionTo(
      'SUSPENDED',
  ...
  describe('list', () => {
    it('returns only the bound tenant\'s users, paginated', async () => {
      const factory = new InMemoryUserRepositoryFactory();
      await factory.forTenant(ORG_1).save(buildUser(oid('user-1')));
      await factory.forTenant(ORG_1).save(buildUser(oid('user-2')));
      await factory.forTenant(ORG_2).save(buildUser(oid('user-3'), ORG_2));

      const page = await factory.forTenant(ORG_1).list(10);

      expect(page.items).toHaveLength(2);
      expect(page.items.map((user) => user.id)).toEqual([oid('user-1'), oid('user-2')]);
