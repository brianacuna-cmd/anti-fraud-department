...
async function seedOrganization(organizations: InMemoryOrganizationRepository, id = oid('org-1')): Promise<void> {
...
describe('createDeleteOrganizationUseCase', () => {
  it('transitions the organization to CANCELLED and sets DeletedAt', async () => {
    ...
    const organization = await deleteOrganization({ auth: PLATFORM_ADMIN, organizationId: oid('org-1') });
  ...
  it('produces the exact same result as calling transitionOrganizationStatus with next=CANCELLED', async () => {
    ...
    await seedOrganization(organizationsForDelete, oid('org-1'));
    const organizationsForTransition = new InMemoryOrganizationRepository();
    await seedOrganization(organizationsForTransition, oid('org-1'));
    ...
    const viaDelete = await deleteOrganization({ auth: PLATFORM_ADMIN, organizationId: oid('org-1') });
  ...
  it('fails identically to PATCH .../status when the organization is already CANCELLED', async () => {
    ...
    await seedOrganization(organizations);
    const { deleteOrganization } = buildUseCases(organizations);
    await deleteOrganization({ auth: PLATFORM_ADMIN, organizationId: oid('org-1') });
