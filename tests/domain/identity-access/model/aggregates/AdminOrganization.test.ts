...
function activeKey(keyId = 'key-1'): AdminKey {
...
function deprecatedKey(keyId = 'key-0'): AdminKey {
  return createAdminKey({
    keyId: createAdminKeyId(keyId),
    publicKey: `pub-${keyId}`,
    status: 'DEPRECATED',
...
function revokedKey(keyId = 'key-revoked'): AdminKey {
  return createAdminKey({
    keyId: createAdminKeyId(keyId),
    publicKey: `pub-${keyId}`,
    status: 'REVOKED',
...
function buildAdminOrganization(keys: readonly AdminKey[] = [activeKey()]): AdminOrganization {
  return AdminOrganization.create({
    id: createAdminOrganizationId('admin-org-1'),
...
describe('AdminOrganization.create', () => {
  ...
  it('rejects a second ACTIVE key as an invariant violation (D31a)', () => {
    expect(() => buildAdminOrganization([activeKey('key-1'), activeKey('key-2')])).toThrow(
      IdentityAccessError,
    );
    expect(() => buildAdminOrganization([activeKey('key-1'), activeKey('key-2')])).toThrow(
...
describe('AdminOrganization.rehydrate', () => {
  it('reconstructs from stored props without re-validating business rules, even with 2 ACTIVE keys', () => {
    const admin = AdminOrganization.rehydrate({
      id: createAdminOrganizationId('admin-org-1'),
      email: createEmail('root@platform.test'),
      keys: [activeKey('key-1'), activeKey('key-2')],
      createdAt: NOW,
      updatedAt: LATER,
    });
...
describe('AdminOrganization#activeKey', () => {
  it('returns the single ACTIVE key', () => {
    const admin = buildAdminOrganization([activeKey('key-1'), deprecatedKey('key-0')]);

    expect(admin.activeKey()?.keyId).toBe('key-1');
  });
...
describe('AdminOrganization#findKey', () => {
  it('finds a key by id', () => {
    ...
    expect(admin.findKey(createAdminKeyId('key-0'))?.status).toBe('DEPRECATED');
  });

  it('returns null when no key matches', () => {
    const admin = buildAdminOrganization();

    expect(admin.findKey(createAdminKeyId('missing'))).toBeNull();
  });
});

describe('AdminOrganization#rotateKey', () => {
  it('demotes exactly one ACTIVE to DEPRECATED and appends the new ACTIVE key (D33)', () => {
    const admin = buildAdminOrganization([activeKey('key-1')]);

    const rotated = admin.rotateKey(activeKey('key-2'), LATER);

    expect(rotated).not.toBe(admin);
    expect(rotated.keys).toHaveLength(2);
    expect(rotated.findKey(createAdminKeyId('key-1'))?.status).toBe('DEPRECATED');
    expect(rotated.findKey(createAdminKeyId('key-1'))?.rotatedAt).toBe(LATER);
    expect(rotated.findKey(createAdminKeyId('key-2'))?.status).toBe('ACTIVE');
    expect(rotated.activeKey()?.keyId).toBe('key-2');
  ...
  it('rejects rotating in a non-ACTIVE new key as an invariant violation', () => {
    ...
    expect(() => admin.rotateKey(deprecatedKey('key-2'), LATER)).toThrow(IdentityAccessError);
  });
});
...
describe('AdminOrganization#revokeKey', () => {
  it('marks the key REVOKED with revokedAt set', () => {
    ...
    const revoked = admin.revokeKey(createAdminKeyId('key-1'), LATER);

    expect(revoked).not.toBe(admin);
    expect(revoked.findKey(createAdminKeyId('key-1'))?.status).toBe('REVOKED');
    expect(revoked.findKey(createAdminKeyId('key-1'))?.revokedAt).toBe(LATER);
    expect(revoked.updatedAt).toBe(LATER);
  });

  it('is terminal — revoking an already-REVOKED key throws an invariant violation', () => {
    const admin = buildAdminOrganization([activeKey('key-1')]).revokeKey(createAdminKeyId('key-1'), LATER);

    expect(() => admin.revokeKey(createAdminKeyId('key-1'), LATER)).toThrow(IdentityAccessError);
  });

  it('throws when the keyId does not exist on the aggregate', () => {
    const admin = buildAdminOrganization();

    expect(() => admin.revokeKey(createAdminKeyId('missing'), LATER)).toThrow(IdentityAccessError);
  });
});
...
describe('AdminOrganization#markPrivateKeyDownloaded', () => {
  it('sets privateKeyDownloadedAt and clears encryptedPrivateKey', () => {
    ...
    const claimed = admin.markPrivateKeyDownloaded(createAdminKeyId('key-1'), LATER);

    const key = claimed.findKey(createAdminKeyId('key-1'));
  ...
  it('throws when the key was already downloaded (one-time, D32a)', () => {
    const admin = buildAdminOrganization([activeKey('key-1')]).markPrivateKeyDownloaded(
      createAdminKeyId('key-1'),
      LATER,
    );

    expect(() => admin.markPrivateKeyDownloaded(createAdminKeyId('key-1'), LATER)).toThrow(
      IdentityAccessError,
    );
  });
});
